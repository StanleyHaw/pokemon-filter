/**
 * Ability name lookup map: PokéAPI ability name → Traditional Chinese name
 */
export const ABILITY_MAP: Record<string, string> = {
  "adaptability": "適應力",
  "aerilate": "飛行皮膚",
  "aftermath": "引爆",
  "analytic": "分析",
  "anger-point": "憤怒穴位",
  "anger-shell": "憤怒甲殼",
  "anticipation": "危險預知",
  "arena-trap": "沙穴",
  "armor-tail": "尾甲",
  "aroma-veil": "芳香幕",
  "as-one": "人馬一體",
  "aura-break": "氣場破壞",
  "bad-dreams": "夢魘",
  "ball-fetch": "撿球",
  "battery": "蓄電池",
  "battle-armor": "戰鬥盔甲",
  "battle-bond": "牽絆變身",
  "beads-of-ruin": "災禍之玉",
  "beast-boost": "異獸提升",
  "berserk": "怒火沖天",
  "big-pecks": "健壯胸肌",
  "blaze": "猛火",
  "bulletproof": "防彈",
  "cheek-pouch": "頰囊",
  "chilling-neigh": "蒼白嘶鳴",
  "chlorophyll": "葉綠素",
  "clear-body": "恆淨之軀",
  "cloud-nine": "無關天氣",
  "color-change": "變色",
  "comatose": "絕對睡眠",
  "commander": "發號施令",
  "competitive": "好勝",
  "compound-eyes": "複眼",
  "contrary": "唱反調",
  "corrosion": "腐蝕",
  "costar": "同臺共演",
  "cotton-down": "棉絮",
  "cud-chew": "反芻",
  "curious-medicine": "怪藥",
  "cursed-body": "詛咒之軀",
  "cute-charm": "迷人之軀",
  "damp": "濕氣",
  "dancer": "舞者",
  "dark-aura": "暗黑氣場",
  "dauntless-shield": "不屈之盾",
  "dazzling": "鮮艷之軀",
  "defeatist": "軟弱",
  "defiant": "不服輸",
  "delta-stream": "德爾塔氣流",
  "desolate-land": "終結之地",
  "disguise": "畫皮",
  "download": "下載",
  "dragons-maw": "龍顎",
  "drizzle": "降雨",
  "drought": "日照",
  "dry-skin": "乾燥皮膚",
  "early-bird": "早起",
  "earth-eater": "食土",
  "effect-spore": "孢子",
  "electric-surge": "電氣製造者",
  "electromorphosis": "電力轉換",
  "embody-aspect": "面影輝映",
  "emergency-exit": "危險回避",
  "fairy-aura": "妖精氣場",
  "filter": "過濾",
  "flame-body": "火焰之軀",
  "flare-boost": "受熱激升",
  "flash-fire": "引火",
  "flower-gift": "花之禮",
  "flower-veil": "花幕",
  "fluffy": "毛茸茸",
  "forecast": "陰晴不定",
  "forewarn": "預知夢",
  "friend-guard": "友情防守",
  "frisk": "察覺",
  "full-metal-body": "金屬防護",
  "fur-coat": "毛皮大衣",
  "gale-wings": "疾風之翼",
  "galvanize": "電氣皮膚",
  "gluttony": "貪吃鬼",
  "good-as-gold": "黃金之軀",
  "gooey": "黏滑",
  "gorilla-tactics": "一猩一意",
  "grass-pelt": "草之毛皮",
  "grassy-surge": "青草製造者",
  "grim-neigh": "漆黑嘶鳴",
  "guard-dog": "看門犬",
  "gulp-missile": "一口導彈",
  "guts": "毅力",
  "hadron-engine": "強子引擎",
  "harvest": "收穫",
  "healer": "治癒之心",
  "heatproof": "耐熱",
  "heavy-metal": "重金屬",
  "honey-gather": "採蜜",
  "hospitality": "款待",
  "huge-power": "大力士",
  "hunger-switch": "飽了又餓",
  "hustle": "活力",
  "hydration": "濕潤之軀",
  "hyper-cutter": "怪力鉗",
  "ice-body": "冰凍之軀",
  "ice-face": "結凍頭",
  "ice-scales": "冰鱗粉",
  "illuminate": "發光",
  "illusion": "幻覺",
  "immunity": "免疫",
  "imposter": "變身者",
  "infiltrator": "穿透",
  "innards-out": "飛出的內在物",
  "inner-focus": "精神力",
  "insomnia": "不眠",
  "intimidate": "威嚇",
  "intrepid-sword": "不撓之劍",
  "iron-barbs": "鐵刺",
  "iron-fist": "鐵拳",
  "justified": "正義之心",
  "keen-eye": "銳利目光",
  "klutz": "笨拙",
  "leaf-guard": "葉子防守",
  "levitate": "飄浮",
  "libero": "自由者",
  "light-metal": "輕金屬",
  "lightning-rod": "避雷針",
  "limber": "柔軟",
  "lingering-aroma": "甩不掉的氣味",
  "liquid-ooze": "污泥漿",
  "liquid-voice": "濕潤之聲",
  "long-reach": "遠隔",
  "magic-bounce": "魔法鏡",
  "magic-guard": "魔法防守",
  "magician": "魔術師",
  "magma-armor": "熔岩鎧甲",
  "magnet-pull": "磁力",
  "marvel-scale": "神奇鱗片",
  "mega-launcher": "超級發射器",
  "merciless": "不仁不義",
  "mimicry": "擬態",
  "minds-eye": "心眼",
  "minus": "負電",
  "mirror-armor": "鏡甲",
  "misty-surge": "薄霧製造者",
  "mold-breaker": "破格",
  "moody": "心情不定",
  "motor-drive": "電氣引擎",
  "moxie": "自信過度",
  "multiscale": "多重鱗片",
  "multitype": "多屬性",
  "mummy": "木乃伊",
  "mycelium-might": "菌絲之力",
  "natural-cure": "自然回復",
  "neuroforce": "腦核之力",
  "neutralizing-gas": "化學變化氣體",
  "no-guard": "無防守",
  "normalize": "一般皮膚",
  "oblivious": "遲鈍",
  "opportunist": "跟風",
  "orichalcum-pulse": "緋紅脈動",
  "overcoat": "防塵",
  "overgrow": "茂盛",
  "own-tempo": "我行我素",
  "parental-bond": "親子愛",
  "pastel-veil": "粉彩護幕",
  "perish-body": "滅亡之軀",
  "pickpocket": "順手牽羊",
  "pickup": "撿拾",
  "pixilate": "妖精皮膚",
  "plus": "正電",
  "poison-heal": "毒療",
  "poison-point": "毒刺",
  "poison-puppeteer": "毒傀儡",
  "poison-touch": "毒手",
  "power-construct": "群聚變形",
  "power-of-alchemy": "化學之力",
  "power-spot": "能量點",
  "prankster": "惡作劇之心",
  "pressure": "壓迫感",
  "primordial-sea": "始源之海",
  "prism-armor": "稜鏡裝甲",
  "propeller-tail": "螺旋尾鰭",
  "protean": "變幻自如",
  "protosynthesis": "古代活性",
  "psychic-surge": "精神製造者",
  "punk-rock": "龐克搖滾",
  "pure-power": "瑜伽之力",
  "purifying-salt": "潔淨之鹽",
  "quark-drive": "夸克充能",
  "queenly-majesty": "女王的威嚴",
  "quick-draw": "速擊",
  "quick-feet": "飛毛腿",
  "rain-dish": "雨盤",
  "rattled": "膽怯",
  "receiver": "接球手",
  "reckless": "捨身",
  "refrigerate": "冰凍皮膚",
  "regenerator": "再生力",
  "ripen": "熟成",
  "rivalry": "鬥爭心",
  "rks-system": "ＡＲ系統",
  "rock-head": "堅硬腦袋",
  "rocky-payload": "搬岩",
  "rough-skin": "粗糙皮膚",
  "run-away": "逃跑",
  "sand-force": "沙之力",
  "sand-rush": "撥沙",
  "sand-spit": "吐沙",
  "sand-stream": "揚沙",
  "sand-veil": "沙隱",
  "sap-sipper": "食草",
  "schooling": "魚群",
  "scrappy": "膽量",
  "screen-cleaner": "除障",
  "seed-sower": "掉出種子",
  "serene-grace": "天恩",
  "shadow-shield": "幻影防守",
  "shadow-tag": "踩影",
  "sharpness": "鋒銳",
  "shed-skin": "蛻皮",
  "sheer-force": "強行",
  "shell-armor": "硬殼盔甲",
  "shield-dust": "鱗粉",
  "shields-down": "界限盾殼",
  "simple": "單純",
  "skill-link": "連續攻擊",
  "slow-start": "慢啟動",
  "slush-rush": "撥雪",
  "sniper": "狙擊手",
  "snow-cloak": "雪隱",
  "snow-warning": "降雪",
  "solar-power": "太陽之力",
  "solid-rock": "堅硬岩石",
  "soul-heart": "魂心",
  "soundproof": "隔音",
  "speed-boost": "加速",
  "stakeout": "蹲守",
  "stall": "慢出",
  "stalwart": "堅毅",
  "stamina": "持久力",
  "stance-change": "戰鬥切換",
  "static": "靜電",
  "steadfast": "不屈之心",
  "steam-engine": "蒸汽機",
  "steelworker": "鋼能力者",
  "steely-spirit": "鋼之意志",
  "stench": "惡臭",
  "sticky-hold": "黏著",
  "storm-drain": "引水",
  "strong-jaw": "強壯之顎",
  "sturdy": "結實",
  "suction-cups": "吸盤",
  "super-luck": "超幸運",
  "supersweet-syrup": "甘露之蜜",
  "supreme-overlord": "大將",
  "surge-surfer": "衝浪之尾",
  "swarm": "蟲之預感",
  "sweet-veil": "甜幕",
  "swift-swim": "悠游自如",
  "sword-of-ruin": "災禍之劍",
  "symbiosis": "共生",
  "synchronize": "同步",
  "tablets-of-ruin": "災禍之簡",
  "tangled-feet": "蹣跚",
  "tangling-hair": "捲髮",
  "technician": "技術高手",
  "telepathy": "心靈感應",
  "tera-shell": "太晶甲殼",
  "tera-shift": "太晶變形",
  "teraform-zero": "歸零化境",
  "teravolt": "兆級電壓",
  "thermal-exchange": "熱交換",
  "thick-fat": "厚脂肪",
  "tinted-lens": "有色眼鏡",
  "torrent": "激流",
  "tough-claws": "硬爪",
  "toxic-boost": "中毒激升",
  "toxic-chain": "毒鎖鏈",
  "toxic-debris": "毒滿地",
  "trace": "複製",
  "transistor": "電晶體",
  "triage": "先行治療",
  "truant": "懶惰",
  "turboblaze": "渦輪火焰",
  "unaware": "純樸",
  "unburden": "輕裝",
  "unnerve": "緊張感",
  "unseen-fist": "無形拳",
  "vessel-of-ruin": "災禍之鼎",
  "victory-star": "勝利之星",
  "vital-spirit": "幹勁",
  "volt-absorb": "蓄電",
  "wandering-spirit": "遊魂",
  "water-absorb": "儲水",
  "water-bubble": "水泡",
  "water-compaction": "遇水凝固",
  "water-veil": "水幕",
  "weak-armor": "碎裂鎧甲",
  "well-baked-body": "焦香之軀",
  "white-smoke": "白色煙霧",
  "wimp-out": "躍躍欲逃",
  "wind-power": "風力發電",
  "wind-rider": "乘風",
  "wonder-guard": "神奇守護",
  "wonder-skin": "奇蹟皮膚",
  "zen-mode": "達摩模式",
  "zero-to-hero": "全能變身",
};

/** Look up the Traditional Chinese name for a PokéAPI ability name */
export function getAbilityNameTw(apiName: string): string {
  return ABILITY_MAP[apiName] ?? "";
}
